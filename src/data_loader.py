from langchain_community.document_loaders import JSONLoader,TextLoader, CSVLoader, DirectoryLoader, PyPDFLoader, PyMuPDFLoader
...
def load_all_documents(data_dir: str) -> List[Any]:
    """ 
    Load all supported files from data directory and conver to Langcahin document structure
    Supported: PDF, TXT, CSV, Excel, Word, JSON
    """

    #Use project root data folder
    data_path = Path(data_dir).resolve()
    print(f"[DEBUG] Data Patg: {data_path}")
    documents = []
    ...
    for pdf_file in pdf_files:
        ...
        try:
            ...
            loaded = loader.load()
            print(f"[DEBUG] Loaded {len(loaded)}")
            documents.extend(loaded)
        except Exception as e:
            print(f"Error Filed to load pdf {pdf_file}:{e}")
